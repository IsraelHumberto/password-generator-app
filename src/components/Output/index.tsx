...
const Output = ({text, onclick}: Props) => {
   const {password} = useContext(PasswordContext)
    ...
    const copyPaswword = () => {
        if(password) {
            ...
            const outputCopiedEl = document.querySelector('.output__copied')
            outputCopiedEl?.setAttribute('style', 'opacity: 1')

            setTimeout(() => {
                outputCopiedEl?.removeAttribute('style')
            }, 2000)
  ...
  return (
    <div className='output'>
        <input className="output__input" placeholder='P4$5w0Rd!' value={password} disabled/> 
        <span className='output__copied'>Copied</span>
        <button className='output__button' onClick={copyPaswword}>
